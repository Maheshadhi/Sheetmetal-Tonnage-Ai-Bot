...
<div style='text-align:center; padding: 30px 0 20px 0;'>
    ...
    <p style='color:#6b7280; font-size:15px; letter-spacing:1px;'>
...
# ─────────────────────────────────────────────
# CLEAN TEXT FOR PDF (removes non-ASCII chars)
# ─────────────────────────────────────────────
def clean(text):
    ...
    # Final safety net — drop anything still non-ASCII
...
def run_calculation(params):
    ...
    base_w,  base_cost  = calc_coating(
        p["base_coating_type"], p["base_coating_micron"], surface_area_used)
    final_w, final_cost_coat = calc_coating(
        p["final_coating_type"], p["final_coating_micron"], surface_area_used)
...
class CostingPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(
            0, 10,
            clean(f"Page {self.page_no()} - AI Sheet Metal Costing Assistant"),
            align="C"
        )
...
def generate_pdf_report(r):
    ...
    pdf.cell(0, 5,
             clean("Generated by AI Sheet Metal Costing Assistant"),
             align="C", ln=True)
    pdf.ln(3)
    ...
    try:
        ...
        sw     = r["strip_width"]
        sl     = r["strip_length"]
        ca     = r["carriage_allowance"]
        sa     = r["stripping_allowance"]
        py_val = ca + sa
        ...
        ax.add_patch(plt.Rectangle(
            (0, 0), sl, sw,
            fill=False, linewidth=2, edgecolor="#1d4ed8"))

        # Carriage zones
        ax.add_patch(plt.Rectangle(
            (0, 0), sl, ca, color="#f59e0b", alpha=0.6, label="Carriage"))
        ax.add_patch(plt.Rectangle(
            (0, sw - ca), sl, ca, color="#f59e0b", alpha=0.6))

        # Stripping zones
        ax.add_patch(plt.Rectangle(
            (0, ca), sl, sa, color="#9ca3af", alpha=0.6, label="Stripping"))
        ax.add_patch(plt.Rectangle(
            (0, sw - ca - sa), sl, sa, color="#9ca3af", alpha=0.6))
        ...
        pdf.cell(0, 8,
                 clean(f"Chart could not be embedded: {chart_err}"),
                 ln=True)
...
def generate_excel_report(r):
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        ws.write("A1", "AI Sheet Metal Costing Report", title_fmt)
        ws.write("A2", "Generated by AI Sheet Metal Costing Assistant", sub_fmt)
        row = 3
        ...
        for section_name, rows in sections.items():
            ...
            ws2 = wb.add_worksheet(safe_name)
...
def answer_query(q: str, r: dict, params: dict):
    ...
    # ── Tonnage ───────────────────────────────
        ...
        reply = (f"Press Tonnage: {r['tonnage']:.2f} T\n\n"
                 ...
                 f"- FOS = 50% of Cutting Force\n"
                 f"- Stripping = 40% of Cutting Force\n"
    ...
    # ── Manufacturing ─────────────────────────
        ...
        reply = (f"Manufacturing Cost: {r['manufacturing_cost']:.6f} EUR\n\n"
                 ...
                 f" (MHR: {r['stamping_mhr']} Labour: {r['stamping_labour_pct']}%)\n"
                 f"- Coating Process: {r['coating_proc_cost']:.6f} EUR"
                 f" (MHR: {r['coating_mhr']} Labour: {r['coating_labour_pct']}%)\n"
                 f"- Inspection: {r['inspection_cost']:.6f} EUR"
                 f" (MHR: {r['insp_mhr']} Labour: {r['insp_labour_pct']}%)\n\n"
...
def display_results(r):
    ...
    ax.add_patch(plt.Rectangle(
        (0, 0), sl, ca, color="#f59e0b", alpha=0.5))
    ax.add_patch(plt.Rectangle(
        (0, sw - ca), sl, ca, color="#f59e0b", alpha=0.5))
    ax.add_patch(plt.Rectangle(
        (0, ca), sl, sa, color="#6b7280", alpha=0.5))
    ax.add_patch(plt.Rectangle(
        (0, sw - ca - sa), sl, sa, color="#6b7280", alpha=0.5))
    ax.add_patch(plt.Rectangle(
        (0, py_v), lx, wx,
    ...
    txk = dict(color="#e2e8f0", fontsize=9,
               ha="center", fontweight="bold")
    ...
    ax.legend(handles=[
        ...
        mpatches.Patch(facecolor="#1e3a5f",
                       edgecolor="#60a5fa", label="Part"),
    ...
    with dl1:
        try:
            ...
            st.download_button(
                label="📄 Download PDF",
                data=pdf_bytes,
                file_name="sheet_metal_costing_report.pdf",
    ...
    with dl2:
        try:
            ...
            st.download_button(
                ...
                data=excel_buf,
                file_name="sheet_metal_costing_report.xlsx",
    ...
    with dl3:
        ...
        st.download_button(
            label="📋 Download CSV",
            data=csv_bytes,
            file_name="sheet_metal_costing_report.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"csv_dl_{id(r)}"
        )
...
with tab2:
    ...
    if "calc_results" in st.session_state:
        st.markdown("<hr class='custom-divider'>", unsafe_allow_html=True)
        st.markdown(
            '<div class="section-header">🤖 Costing Assistant Chatbot</div>',
            unsafe_allow_html=True)
        st.markdown("""
        <div class="info-card">
